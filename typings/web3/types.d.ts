...
export declare interface Utils {
  BN: BN.BN // TODO only static-definition
  ...
  numberToHex(val: number | BN.BN): string
  checkAddressChecksum(address: string): boolean
  fromAscii(val: string): string
  fromDecimal(val: string | number | BN.BN): string
  fromUtf8(val: string): string
  fromWei(val: string | number | BN.BN, unit: Unit): string | BN.BN
  hexToBytes(val: string): number[]
  hexToNumber(val: string | number | BN.BN): number
  hexToNumberString(val: string | number | BN.BN): string
  ...
  toWei(val: string | number | BN.BN, unit: Unit): string | BN.BN
  unitMap: any
}
